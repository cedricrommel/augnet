""" Differentiable augmentations for vision tasks

This file was copied from https://github.com/moskomule/dda and modified
...
from .functional import (
    shear_x, shear_y, translate_x, translate_y, hflip, vflip, rotate, invert,
    solarize, posterize, gray, contrast, auto_contrast, saturate, brightness,
    hue, sample_pairing, equalize, sharpness
)
...
class _Operation(nn.Module):
    """ Base class of differantiable augmentation operations (tailored to
    vision experiments here).

    Parameters
    ----------
    operation: callable
        Functional defining how the augmentation transforms the input data.
    initial_magnitude: float, optional
        Initial value of the magnitude parameter. Defaults to None (no
        magnitude).
    initial_probability: float, optional
        Initial value of the probability parameter. Default to 0.5.
    magnitude_range: tuple
        Minimum and maximaum magnitude values allowed (used for clamping).
        Defaults to None (no clamping).
    probability_range: tuple
        Minimum and maximaum probability values allowed (used for clamping).
        Defaults to None (no clamping).
    temperature: float, optional
        Temperature for RelaxedBernoulli distribution used during training.
        Defaults to 0.1.
    flip_magnitude: bool, optional
        Whether to consider positive and negative magnitude values. Should be
        True for geometric augmentations like rotations, translations, ...
        Defaults to False.
    bounding_mag: bool, optional
        Whether to sample effective magnitude at forward between bounds
        - mag and + mag set by the magnitude parameter. Defaults to False.
    debug: bool, optional
        If True, check if image is in [0, 1].
    """
    ...
    def __init__(self,
        ...
        if self.probability_range is None:
            ...
            assert (
                0 <= self.probability_range[0] < self.probability_range[1] <= 1
            )
        ...
        self.bounding_mag = bounding_mag and (self._magnitude is not None)
    ...
    def forward(self,
        ...
        if self.training:
            return (
                mask * self.operation(input, mag) + (1 - mask) * input
            ).clamp_(0, 1)
        else:
            mask.squeeze_()
            output = input
    ...
    def __repr__(self) -> str:
        ...
        prob_state = (
            'frozen' if self.probability_range is None else 'learnable'
        )
        s += f"(probability={self._py_probability:.3f} ({prob_state}), "
        if self.magnitude is not None:
            mag_state = (
                'frozen' if self.magnitude_range is None else 'learnable'
            )
            s += (
                f"{' ' * len(s)} magnitude={self._py_magnitude:.3f} "
                f"({mag_state}), "
            )
...
class MagHorizontalFlip(_Operation):
    """Same as HorizontalFlip, but where the probability of flipping is
    ...
    def __init__(self,
                 ...
                 debug: bool = False):
        super(MagHorizontalFlip, self).__init__(
            operation=hflip,
