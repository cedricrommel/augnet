""" This file was copied from https://github.com/moskomule/dda
...
__all__ = ['shear_x', 'shear_y', 'translate_x', 'translate_y', 'hflip',
           'vflip', 'rotate', 'invert', 'solarize', 'posterize',
           'gray', 'contrast', 'auto_contrast', 'saturate', 'brightness',
           'hue', 'equalize', 'sharpness']
...
def pil_function(func):
    ...
    def inner(*args):
        if not isinstance(args[0], PILImage):
            raise RuntimeError(
                'img is expected to be PIL.Image, '
                f'but got {type(args[0])} instead!'
            )
        return func(*args)
