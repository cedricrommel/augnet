"""
`functional` contains deterministic functions behind differentiable
augmentations for computer vision tasks.
image tensor `img` is expected to be CxHxW or BxCxHxW and its range should
be [0, 1] `mag=0` expects no transformation
...
__all__ = ['shear_x', 'shear_y', 'translate_x', 'translate_y', 'hflip',
           'vflip', 'rotate', 'invert', 'solarize', 'posterize', 'gray',
           'contrast', 'auto_contrast', 'saturate', 'brightness', 'hue',
           'sample_pairing', 'equalize', 'sharpness']
...
def tensor_function(func):
    ...
    def inner(*args):
        ...
        if not torch.is_tensor(img):
            raise RuntimeError(
                'img is expected to be torch.Tensor, '
                f'but got {type(img)} instead'
            )
        ...
        if (
            torch.is_tensor(mag) and
            mag.nelement() != 1 and
            mag.size(0) != img.size(0)
        ):
...
def contrast(img: torch.Tensor,
             mag: torch.Tensor) -> torch.Tensor:
    mean = _gray(img * 255).flatten(1).mean(dim=1).add(0.5).floor().view(
                -1, 1, 1, 1
            ) / 255
...
def auto_contrast(img: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        output = (
            torch.arange(256, device=img.device, dtype=img.dtype) - min
        ) * (255 / (max - min + 0.1))
        output = output.floor().gather(
            1, reshaped.long()).reshape_as(img) / 255
...
def equalize(img: torch.Tensor,
             _=None) -> torch.Tensor:
    # see https://github.com/python-pillow/Pillow/blob/master/src/PIL/
    # ImageOps.py#L319
    with torch.no_grad():
        # BCxHxW
        reshaped = img.clone().flatten(0, 1).clamp_(0, 1) * 255
        size = reshaped.size(0)  # BC
        # 0th channel [0-255], 1st channel [256-511],
        # 2nd channel [512-767]...(BC-1)th channel
        shifted = reshaped + 256 * torch.arange(
            0, size, device=reshaped.device,
            dtype=reshaped.dtype
        ).view(-1, 1, 1)
        # channel wise histogram: BCx256
        histogram = shifted.histc(
            size * 256, 0, size * 256 - 1
        ).view(size, 256)
        ...
        cdf = torch.cat(
            [cdf.new_zeros((cdf.size(0), 1)), cdf], dim=1
        )[:, :256] + (step / 2).floor_()
        # to avoid zero-div, add 0.1
        output = (
            cdf / (step + 0.1)
        ).floor_().view(-1)[shifted.long()].reshape_as(img) / 255
    return ste(output, img)
